...
int main(int argc, char* argv[]) {
    sf::RenderWindow window(sf::VideoMode(1280, 720), "Dial's Algorithm Visualizer");
    sf::Font font1;
    ...
    while (window.isOpen()) {
        ...
        if (isCalculating && startVertex != nullptr && endVertex != nullptr) {
            if (isFirstRun) {
                ...
                int verticesCount = unexplored.size();
                int maxWeight = manager.getMaxWeight(unexplored);
                buckets.resize(verticesCount * maxWeight + 1);

                std::cout << "Connections: " << maxWeight << std::endl;
                std::cout << "Vertices: " << verticesCount << std::endl;
                std::cout << "Resize: " << verticesCount * maxWeight + 1 << std::endl;

                std::vector<Vertex* >::iterator startElement = std::find_if(unexplored.begin(), 
                    unexplored.end(), [&startVertex](const Vertex* node) {
                ...
                Vertex* firstVertex = unexplored.front();
                firstVertex->minDistanceFromSrc = 0;

                explored.push_back(firstVertex);
            ...
            if (vertexLoopRunning) {
                ...
                if (unexploredIterator == unexplored.end()) {
                    ...
                    auto foundRelatedVertex = std::find_if(topExplored->neighbors.begin(), 
                        topExplored->neighbors.end(), [target](const std::pair<std::string, int>& neighbor) {
                    ...
                    if (foundRelatedVertex != topExplored->neighbors.end()) {
                        ...
                        if (totalDistance < (*unexploredIterator)->minDistanceFromSrc) {
                            ...
                            buckets[totalDistance].push_back((*unexploredIterator));
                        }

                        for (int i = 0; i < buckets.size(); i++) {
                            std::vector<Vertex*>& currentBucket = buckets.at(i);

                            if (currentBucket.empty())
                                continue;

                            Vertex* currentVertex = currentBucket.at(0);

                            if (currentVertex->minDistanceFromSrc == 0)
                                continue;

                            explored.push_back(currentVertex);
        ...
        for (int i = 0; i < vertices->size(); i++) {
            ...
            if (!isCalculating) {
                ...
                else if (pathPtr != nullptr && startVertex != nullptr && endVertex != nullptr && 
                    currentVertex != startVertex && currentVertex != endVertex)
        ...
        if (unexplored.empty() && isCalculating) {
            ...
            auto foundDijkstraResult = std::find_if(explored.begin(), explored.end(), 
                [&endVertex](const Vertex* currentVertex) {
...
void addVertexFromUser(GraphManager& manager, std::string& inputText) {
    ...
    manager.createVertex(inputText, std::vector<std::pair<std::string, int>>{}, 
        newVertexPosX, newVertexPosY);
}
